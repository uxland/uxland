...
export const displayMetaInformation = (): void => {
  ...
  console.log(`%c|-- Browser version: ${getBrowser().name} ${getBrowser().version}`, 'color:#388e3c');
  ...
  console.log(`%c|-- Lit-element version: ${(window as any).litElementVersions[0]}`, 'color:#388e3c');
  ...
  console.log(
    `%c|-- Orientation: ${window.orientation === 0 ? 'Portrait' : window.orientation === 90 ? 'landscape' : 'unknown'}`,
    'color:#388e3c'
  );
