...
export class RegionAdapterRegistry {
    ...
    registerDynamicAdapterFactory(factory: DynamicFactory){
        if(this.dynamicFactories.indexOf(factory) === -1)
            this.dynamicFactories.push(factory);
    }

    getDynamicFactory(host: Element): adapterFactory{
        return this.dynamicFactories.reduce((previousValue, currentValue) => previousValue || currentValue(host), <adapterFactory>null);
    }
