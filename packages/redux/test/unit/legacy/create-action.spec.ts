...
import { createAction } from "../../../legacy";
...
describe("create action fixture", () => {
  ...
  it("uses return value as payload", () => {
    ...
    const foobar = { foo: "bar" };
    const action = actionCreator(foobar);
    expect(action).to.deep.equal({ type, payload: foobar });
  });
  it("throws an error if payloadCreator is not a function, undefined, null", () => {
    const wrongTypePayloadCreators = [1, false, "string", {}, []];
    wrongTypePayloadCreators.forEach((x) => {
      expect(() => createAction(type, x as any)).throw(
  ...
  it("sets error to true if payload is an Error object", () => {
    ...
    expect(isFSA(noErrAction)).to.equal(true);
  });
  ...
  it("sets payload only when defined", () => {
    ...
    expect(explicitUndefinedAction).to.deep.equal({
      type,
    });
  ...
  it("sets error to true if payloadCreator return an Error object", () => {
    const errObj = new TypeError("this is an error");
