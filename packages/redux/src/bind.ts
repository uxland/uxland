...
const getAllStores = (watchers: {
  [key: string]: PropertyWatch;
}): Store<any, any>[] => uniq(map((x) => x.store, values(watchers)));
const mapWatchers = (watchersMap: { [key: string]: PropertyWatch }): any =>
  values(watchersMap);
...
const initializeValues = (context: any) => (stores: Store<any, any>[]): any => {
  ...
  storeWatchers.forEach((watcher: any[]) =>
    watcher.forEach(
      (x) => (context[x.name] = x.selector.call(context, x.store.getState()))
    )
  );
  return stores;
};
...
const rejectUnchanged: (changes: PropertyState[]) => PropertyState[] = reject<
  PropertyState
>((x) => x?.old === x?.current);
  ...
  const update = (): any =>
    pipe(
      (watchers: PropertyWatch[]) =>
        getProperties(store.getState(), context)(watchers),
      rejectUnchanged,
      updateProperties(context),
      nop
    )(watchers);
  return (): Debouncer =>
    Debouncer.debounce(debounceJob, timeOut.after(16), update);
};
const listener = (context: any) => (store: Store): Unsubscribe =>
  store.subscribe(listen(context, store));
const subscribe = (context: any): any =>
  map<Store, Unsubscribe>(listener(context));
const storeSubscriptions = (context: any) => (
