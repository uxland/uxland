...
describe('create async reducer fixture path resolver', () => {
  ...
  test('handling action failed sets error properties', () => {
    ...
    newState = reducer(state, { type: failedAction, payload: 34 });
    expect(newState).toEqual({
      data: { isFetching: false, didInvalidate: false, error: true, errorDescription: '34', exceptions: [34] }
    });
  ...
  test('actions should set timestamp if supplied in action', () => {
    ...
    expect(state).not.toBe(newState);
    expect(newState).toEqual({
      data: { isFetching: false, didInvalidate: false, error: false, state: 55, timestamp: timestamp }
    });
    newState = reducer(state, { type: failedAction, payload: 'fail', timestamp });
    expect(state).not.toBe(newState);
    expect(newState).toEqual({
    ...
    expect(state).not.toBe(newState);
    expect(newState).toEqual({
      data: { isFetching: false, didInvalidate: false, error: false, state: 15, elapsed: 35, timestamp: timestamp }
    });
    newState = reducer(state, { type: invalidatedAction, payload: 35, timestamp });
    expect(newState).toEqual({
      data: { isFetching: false, didInvalidate: true, error: false, state: 15, timestamp: timestamp }
    });
  ...
  test('actions should set timestamp if timestamp resolver option', () => {
    ...
    expect(state).not.toBe(newState);
    expect(newState).toEqual({
      data: { isFetching: false, didInvalidate: true, error: false, state: { value: 15 }, timestamp }
    });
