...
export function fuse(config: FuseConfig) {
  const workspaces = (config.workspaces || ['packages']).reduce((collection, w) => {
    if (w.name) collection.push({name: w.name, baseStyles: config.baseStyles}, w);
  ...
  return fusebox({
    ...
    hmr: {
      plugin: join(workspaceRoot, 'node_modules/@uxland/project-tools/fuse/hmr-plugin.ts'),
    },
    entry: config.entry,
    env: config.env || {},
    target: 'browser',
