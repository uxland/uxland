import { subscribeOnce } from "@uxland/event-aggregator";
...
describe("Given a response", () => {
  it("should throw an error and publish an INVALID_REQUEST_EVENT", (done) => {
    ...
    subscribeOnce(INVALID_REQUEST_EVENT, eventHandler);
  ...
  describe("and status is 401", () => {
    it("should publish an INVALID_CREDENTIALS_EVENT", (done) => {
      const response: AxiosResponse = {
        status: 401,
        statusText: "",
        headers: undefined,
      ...
      const eventHandler = jest.fn();
      subscribeOnce(INVALID_CREDENTIALS_EVENT, eventHandler);
      handleErrors(response)
        .then()
        .catch()
        .finally(() => {
  ...
  //     expect(await handleErrors(response)).toEqual(response.data);
  //     done();
  //   });
  // });
