...
*/
import './boot';
import { pathFromUrl } from './resolve-url';

export const useShadow = !((window as any).ShadyDOM);
export const useNativeCSSProperties = Boolean(!(window as any).ShadyCSS || (window as any).ShadyCSS.nativeCss);
export const useNativeCustomElements = !((window as any).customElements?.polyfillWrapFlushCallback);
 ...
 */
export let rootPath = pathFromUrl(document.baseURI || (window as any).location.href);
 ...
 */
export let sanitizeDOMValue = (window as any).Polymer && (window as any).Polymer.sanitizeDOMValue || undefined;
