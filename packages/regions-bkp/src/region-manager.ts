...
class Registry {
  ...
  registerView(regionName: string, key: string, view: ViewDefinition) {
    this.viewsByRegion[regionName] = [...(this.viewsByRegion[regionName] || []), { key: key, view: view }];
  }
...
export class RegionManager implements IRegionManager {
  ...
  remove(region: string | IRegion): IRegion {
    ...
    if (typeof region === 'object') {
      regionName = Object.keys(this._regions).find(regionName => this._regions[regionName] === region);
      if (regionName) {
        result = region;
