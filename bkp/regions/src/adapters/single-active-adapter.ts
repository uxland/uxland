...
export class SingleActiveAdapter extends AdapterBase {
  async activateView(view: HTMLElement & ViewComponent): Promise<any> {
    ...
    for (let v of activeViews) await this.host.uxlRegion.deactivate(v);
  ...
  async viewAdded(view: ViewDefinition) {
    if (!this.host.uxlRegion.currentActiveViews.length && view.isDefault) await this.host.uxlRegion.activate(view);
  }
}
